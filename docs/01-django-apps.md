...
| The apps (24 `INSTALLED_APPS` entries, 21 of them directories) | all features | `cmweb-app` |
...
"24" counts the entries under the `# CMWEB apps` comment in
`INSTALLED_APPS`. Only **21** of them are directories in `cmweb-app/` —
`inlines`, `commit_message_checker` and `rest_framework_swagger` are installed
from Sony-internal or third-party packages, which is why `local-cmweb/stubs/`
