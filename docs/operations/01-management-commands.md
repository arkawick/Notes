...
That job runs `bin/run_commands.sh`, which does a fresh `repo sync`, writes its
own `secure.py` and `settings_management.py`, and runs against **the same RDS
instance the web servers read**. See [03](03-jenkins-jobs.md).
